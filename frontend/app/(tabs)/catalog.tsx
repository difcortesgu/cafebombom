...
export default function CatalogScreen() {
    ...
    const { suppliers, ingredients, units, hydrate: hydrateInventory } = useInventoryStore();
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            ...
            <ThemedText>{t('catalog.subtitle')}</ThemedText>
            ...
            {section === 'ingredients' ? (
                <>
                            ...
                            return (
                                <View key={item.id} style={[styles.listItem, { borderColor: palette.border }]}>
                                    <View style={styles.listTextWrap}>
                                        ...
                                        {isLow ? <ThemedText style={[styles.smallText, { color: palette.warning }]}>{t('products.ingredients.lowStock')}</ThemedText> : null}
                                    </View>
                                    <View style={styles.inlineActions}>
                                        <ThemedButton
                                            variant="secondary"
                    ...
                    {units.length > 0 ? (
                        <ThemedCard style={styles.card}>
                            <ThemedText type="subtitle">{t('inventory.units.title')}</ThemedText>
                            {units.map((unit) => (
                                <View key={unit.id} style={[styles.listItem, { borderColor: palette.border }]}>
