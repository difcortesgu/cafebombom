...
export function seedDefaults(db: ExpoSQLiteDatabase<typeof schema>) {
  const owner = db.select({ id: users.id })
    .from(users)
    .where(eq(users.name, 'Dueno'))
    .get();

  if (!owner) {
    // Hashes PINs using bcryptjs with 10 salt rounds
    db.insert(users).values([
      { name: 'Dueno', role: 'owner', pinHash: hashPin('1234') },
      { name: 'Personal', role: 'staff', pinHash: hashPin('2222') },
    ]).run();
  }

  const defaultCategories = ['Cafe', 'Te', 'Pasteleria', 'Snacks'];
  ...
  if (total === 0) {
    const espresso = db.select({ id: ingredients.id })
      .from(ingredients)
      .where(eq(ingredients.name, 'Cafe espresso en grano'))
      .get();
    const milk = db.select({ id: ingredients.id })
      .from(ingredients)
      .where(eq(ingredients.name, 'Leche'))
      .get();
    const teaBlend = db.select({ id: ingredients.id })
      .from(ingredients)
      .where(eq(ingredients.name, 'Mezcla de te'))
      .get();
    const pastryDough = db.select({ id: ingredients.id })
      .from(ingredients)
      .where(eq(ingredients.name, 'Masa de pasteleria'))
      .get();

    if (!espresso) {
      db.insert(ingredients).values({ name: 'Cafe espresso en grano', unit: 'grams', quantity: 0, lowStockThreshold: 500 }).run();
    }
    if (!milk) {
      db.insert(ingredients).values({ name: 'Leche', unit: 'liters', quantity: 0, lowStockThreshold: 2 }).run();
    }
    if (!teaBlend) {
      db.insert(ingredients).values({ name: 'Mezcla de te', unit: 'grams', quantity: 0, lowStockThreshold: 400 }).run();
    }
    if (!pastryDough) {
      db.insert(ingredients).values({ name: 'Masa de pasteleria', unit: 'grams', quantity: 0, lowStockThreshold: 1000 }).run();
    }
    ...
    const coffeeCategory = db.select({ id: categories.id })
      .from(categories)
      .where(eq(categories.name, 'Cafe'))
      .get();
    const teaCategory = db.select({ id: categories.id })
      .from(categories)
      .where(eq(categories.name, 'Te'))
      .get();
    const pastryCategory = db.select({ id: categories.id })
      .from(categories)
      .where(eq(categories.name, 'Pasteleria'))
      .get();

    if (coffeeCategory && teaCategory && pastryCategory && espressoSeed && milkSeed && teaBlendSeed && pastryDoughSeed) {
      db.insert(products).values([
        { name: 'Capuchino', categoryId: coffeeCategory.id, price: 4.5 },
        { name: 'Latte', categoryId: coffeeCategory.id, price: 4.25 },
        { name: 'Te tailandes con leche', categoryId: teaCategory.id, price: 3.9 },
        { name: 'Croissant de mantequilla', categoryId: pastryCategory.id, price: 2.8 },
      ]).run();

      const cappuccino = db.select({ id: products.id }).from(products).where(eq(products.name, 'Capuchino')).get();
      const latte = db.select({ id: products.id }).from(products).where(eq(products.name, 'Latte')).get();
      const thaiMilkTea = db.select({ id: products.id }).from(products).where(eq(products.name, 'Te tailandes con leche')).get();
      const butterCroissant = db.select({ id: products.id }).from(products).where(eq(products.name, 'Croissant de mantequilla')).get();
  ...
  if (discountsTotal === 0) {
    ...
    db.insert(discounts)
      .values([
        { name: 'Gran apertura 5%', scope: 'global', productId: null, type: 'percentage', value: 5, startsAt: now, endsAt: null, isActive: true },
        { name: 'Hora feliz $1', scope: 'global', productId: null, type: 'fixed', value: 1, startsAt: now, endsAt: null, isActive: true },
      ])
      .onConflictDoNothing()
      .run();
  }
